...
@ObjectClassDefinition(
        ...
        description = "Configuration to launch an embedded JCR Repository and provide it as a SlingRepository and"
                + " a standard JCR Repository. In addition, if the registration URL is not empty, the"
                + " repository is registered as defined.")
@interface OakSlingRepositoryManagerConfiguration {
    ...
    @AttributeDefinition(
            ...
            description = "Name of the workspace to use by default if not is given in any of the login methods."
                    + " This name is used to implement the SlingRepository.getDefaultWorkspace() method."
                    + " If this name is empty, a null value is used in JCR calls so that the default"
                    + " workspace provided by the JCR repository is used.")
    ...
    @AttributeDefinition(
            ...
            description = "Whether to enable or disable the SlingRepository.loginAdministrative method. The default"
                    + " is 'true'. See"
    ...
    @AttributeDefinition(
            ...
            description = "Whether the query result size should return an estimation (or -1 if disabled) for large"
                    + " queries")
